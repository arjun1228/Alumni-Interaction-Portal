...
import { MessageSquare, Heart, Share2, Award, Briefcase, Lightbulb, Send, Image } from 'lucide-react';
import { Post, UserRole, User, Comment } from '../types';
import { createPost, likePost } from '../services/api';
...
export const Feed: React.FC<FeedProps> = ({ posts, setPosts, currentUser }) => {
  const [filter, setFilter] = useState<'ALL' | 'ACHIEVEMENT' | 'ADVICE' | 'GENERAL'>('ALL');
  const [newPostContent, setNewPostContent] = useState('');
  const [newPostType, setNewPostType] = useState<'ACHIEVEMENT' | 'ADVICE' | 'GENERAL'>('GENERAL');
              ...
              <form onSubmit={handleCreatePost}>
                ...
                <div className="flex justify-between items-center mt-3">
                  <div className="flex gap-2">
                    ...
                    <button type="button" className="text-slate-400 hover:text-emerald-600 p-1">
          ...
          <div key={post.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
            ...
            {isExpanded && (
              <div className="bg-slate-50 px-5 pb-5 border-t border-slate-100 animate-in fade-in duration-200">
                ...
                <form onSubmit={(e) => handlePostComment(e, post.id)} className="mt-4 flex gap-2">
