...
import { MapPin, Briefcase, Calendar, Building2, X, Globe, Plus, Search } from 'lucide-react';
import { JobListing, UserRole, User } from '../types';
...
export const Jobs: React.FC<JobsProps> = ({ jobs, setJobs, currentUser }) => {
  ...
  const [newType, setNewType] = useState<'INTERNSHIP' | 'FULL_TIME' | 'PART_TIME'>('INTERNSHIP');
  ...
  const handlePostJob = async (e: React.FormEvent) => {
    ...
    const jobData = {
      ...
      postedDate: 'Just now',
      skills: ['General'], // Simplified for demo
      link: newLink
    };
          ...
          >
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-400">
                <img src={job.postedBy.avatar} className="w-5 h-5 rounded-full" alt="poster" />
                <span>Posted by {job.postedBy.name} • {job.postedDate}</span>
              </div>
            </div>
          </div>
        )) : (
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                <div className="flex gap-4 text-sm text-slate-600 border-b border-slate-100 pb-4">
                  ...
                  <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> Posted {selectedJob.postedDate}</span>
                </div>
            ...
            <form onSubmit={handlePostJob} className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select value={newType} onChange={e => setNewType(e.target.value as any)} className="w-full bg-slate-50 text-slate-900 border rounded-lg p-2.5">
                ...
                <div>
                  ...
                  <input type="url" placeholder="https://..." value={newLink} onChange={e => setNewLink(e.target.value)} className="w-full bg-slate-50 text-slate-900 border rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500" />
                </div>
              </div>
