...
import { School, Briefcase, GraduationCap, ArrowRight, Loader2, CheckCircle, AlertCircle, Mail, ShieldCheck, RefreshCw, ChevronRight } from 'lucide-react';
...
type AuthView = 'LOGIN' | 'REGISTER' | 'VERIFY_UG' | 'VERIFY_GRAD';

export const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin }) => {
  const [view, setView] = useState<AuthView>('LOGIN');
  const [activeTab, setActiveTab] = useState<UserRole>(UserRole.UNDERGRADUATE);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Simple auth simulation via backend: check if user exists
      let user;
      ...
      if (user.role !== activeTab) {
        setError(`Please switch to the ${user.role === UserRole.UNDERGRADUATE ? 'Student' : user.role === UserRole.GRADUATE ? 'Alumni' : 'Admin'} login tab.`);
        return;
      }
      onLogin(user);
    } catch (err: any) {
      ...
      if (err.message && (err.message.includes('Failed to fetch') || err.message.includes('NetworkError'))) {
        setError('Cannot connect to server. Is the backend running? (npm run server)');
      } else {
        setError('Login failed or user not found. For demo, try creating an account.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const newUser: User = {
        ...
        // Default Mock Data for new Profile Features
  ...
  const completeRegistration = async () => {
    ...
    setIsLoading(true);
    try {
      const savedUser = await loginUser({ ...pendingUser, password }); // Registering
      onLogin(savedUser);
    } catch (error) {
  ...
  // Render Verification Screen for Undergraduates
  if (view === 'VERIFY_UG') {
    return (
  ...
  // Render Verification Screen for Graduates
  if (view === 'VERIFY_GRAD') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center animate-in fade-in zoom-in duration-300">
        ...
        <div className="hidden md:flex bg-slate-900 relative overflow-hidden order-1 md:order-2 flex-col justify-center p-12 text-white">
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div
                onClick={() => { setActiveTab(UserRole.ADMIN); setError(''); setView('LOGIN'); }}
                className={`group cursor-pointer p-6 rounded-2xl border transition-all duration-300 relative overflow-hidden ${activeTab === UserRole.ADMIN
                  ? 'bg-white text-slate-900 border-white shadow-xl translate-x-4'
                  : 'bg-white/10 hover:bg-white/20 border-white/20 text-white'
                  }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${activeTab === UserRole.ADMIN ? 'bg-slate-100 text-slate-800' : 'bg-white/20'
                      }`}>
                      <ShieldCheck className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="font-bold text-lg">College Admin</h3>
                      <p className={`text-xs mt-1 ${activeTab === UserRole.ADMIN ? 'text-slate-600' : 'text-indigo-200'}`}>
                        Manage Students & activities
                      </p>
                    </div>
                  </div>
                  {activeTab === UserRole.ADMIN && <CheckCircle className="w-6 h-6 text-slate-800" />}
                </div>
              </div>
            </div>
          </div>
