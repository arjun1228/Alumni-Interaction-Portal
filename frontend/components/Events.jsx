...
import { createEvent, rsvpEvent, uploadImage, deleteEvent } from '../services/api';
   ...
   return (
      <div className="space-y-6">
         ...
         <div className="flex justify-between items-center">
            ...
            {(currentUser.role === UserRole.GRADUATE || currentUser.role === UserRole.ADMIN) && (
               <button
               ...
               >
                  ...
                  <div className="p-5">
                     ...
                     <button className="w-full mt-4 bg-white border border-indigo-600 text-indigo-600 py-2 rounded-lg text-sm font-medium hover:bg-indigo-50 transition-colors">
                        {(currentUser.role === UserRole.GRADUATE || currentUser.role === UserRole.ADMIN) ? 'View Details' : 'Register Now'}
                     </button>
                  </div>
               </div>
            ))}
               ...
               <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-200">
                  ...
                  <div className="p-8">
                     ...
                     <div className="flex justify-end gap-3 border-t border-slate-100 pt-6">
                        ...
                        <button 
                           onClick={() => {
                              if (currentUser.role !== UserRole.GRADUATE && currentUser.role !== UserRole.ADMIN) {
                                 handleRegisterForEvent(selectedEvent.id || selectedEvent._id);
                              } else {
                                 setSelectedEvent(null);
                              }
                           }}
                        ...
                        >
                           {isRegistering ? 'Registering...' : (currentUser.role === UserRole.GRADUATE || currentUser.role === UserRole.ADMIN) ? 'Close Info' : 'Confirm Registration'}
                        </button>
                     </div>
                  </div>
               </div>
