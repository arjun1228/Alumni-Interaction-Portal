            ...
            {user.projects && user.projects.length > 0 ? (
              <div className="space-y-4">
                {user.projects.map((project) => (
                  <div key={project.id || project._id} className="p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-indigo-200 transition-colors">
              ...
              <div className="space-y-4">
                ...
                <div className="border-t border-slate-100 pt-4">
                  ...
                  <div className="space-y-3 mb-4">
                    {editProjects.map(p => (
                      <div key={p.id || p._id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-200">
                        <div>
                          <div className="font-bold text-sm">{p.title}</div>
                          <div className="text-xs text-slate-500">{(p.technologies || []).join(', ')}</div>
                        </div>
                        <button type="button" onClick={() => handleRemoveProject(p.id || p._id)} className="text-red-400 hover:text-red-600">
